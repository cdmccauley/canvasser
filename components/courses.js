...
function Course(props) {
  ...
  const handleCourseCheckboxChange = (event) => {
    ...
    props.setActiveCourses(
      event.target.checked
        ? [...props.activeCourses, props.course.code]
        : [
            ...props.activeCourses.filter(
              (courseCode) => courseCode !== props.course.code
            ),
          ]
    );
    localStorage.setItem(
      "activeCourses",
      event.target.checked
        ? JSON.stringify([...props.activeCourses, props.course.code])
        : JSON.stringify([
            ...props.activeCourses.filter(
              (courseCode) => courseCode !== props.course.code
            ),
...
export default function Courses(props) {
  const classes = useStyles();
  const [activeCourseChanges, setActiveCourseChanges] = useState([
    ...props.activeCourses,
  ]);
  ...
  const handleCoursesMenuClose = () => {
    // temp fix to prevent queue re-render while selecting courses
    props.setActiveCourses(activeCourseChanges);
                  ...
                  <Course
                    key={
                      Object.keys(props.courses)[
                        Object.values(props.courses).indexOf(course)
                      ]
                    }
                    courseId={
                      Object.keys(props.courses)[
                    ...
                    course={course}
                    activeCourses={activeCourseChanges}
                    setActiveCourses={setActiveCourseChanges}
                  />
                ))}
            </FormGroup>
          </FormControl>
