...
export default function Priorities(props) {
  ...
  const handleDownwardAction = (level, index) => {
    let stage = [...props.priorities];
    level === stage.length - 1
      ? stage.push([props.priorities[level][index]])
      : stage[level + 1].push(props.priorities[level][index]);
    stage[level].splice(index, 1);
    if (stage[level].length === 0) stage.splice(level, 1);
    localStorage.setItem("priorities", JSON.stringify([...stage]));
  ...
  const handleAddAction = (event) => {
    if (newPriority) {
      let stage = [...props.priorities];
      stage.length > 0
        ? stage[stage.length - 1].push(newPriority)
        : stage.push([newPriority]);
      localStorage.setItem("priorities", JSON.stringify([...stage]));
      props.setPriorities([...stage]);
      setNewPriority("");
    }
  ...
  return (
    <React.Fragment>
      ...
      >
        {props.priorities.map((level) =>
          level.map((priority) => (
            <ListItem
              divider={
                level.indexOf(priority) === level.length - 1 &&
                props.priorities.indexOf(level) < props.priorities.length - 1
                  ? true
                  : false
              }
            >
              <InputBase
                ...
                onChange={(event) =>
                  handlePriorityChange(
                    event,
                    props.priorities.indexOf(level),
                    level.indexOf(priority)
                  )
                }
              />
